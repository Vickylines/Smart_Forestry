...
**当前正式版：v0.3.0。** [下载 APK](https://github.com/Vickylines/Smart_Forestry/releases/tag/v0.3.0)。

设置页新增可跳转的项目仓库地址。修复重查无匹配时旧科属残留、同株后续照片覆盖已补全科属，以及移除照片期间仍可开始其他采集操作的问题。详见[正式版验收记录](docs/Release0.3.0验收记录.md)。

保留全球分类库查询、异名与园艺基础分类匹配，以及旧记录补查。Beta 4 固定 100 名称中 95 个返回完整科属，**尚未达到或验证全球 99% 覆盖**；该历史测量不是本次照片识别准确率。默认字号保持不变，详见[Beta 4 验收记录](docs/Beta4验收记录.md)。
...
7. 在设置页点击“项目仓库”，由系统浏览器或已安装的 GitHub App 打开源码与版本下载页面。
...
本正式版通过 GitHub Releases 分发 APK，未上架应用商店。详见[中英文发布说明](docs/releases/v0.3.0.md)和[本轮验收记录](docs/Release0.3.0验收记录.md)。反馈问题时请说明应用版本、机型与系统、复现步骤及预期／实际结果；不要附带密钥或私人照片。
