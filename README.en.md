...
**Current stable release: v0.3.0.** [Download the APK](https://github.com/Vickylines/Smart_Forestry/releases/tag/v0.3.0).

Settings now opens the project repository through an external browser or GitHub app. Fixes stale taxonomy after an unsuccessful match, loss of enriched taxonomy when a later photo scores higher, and overlapping capture actions during photo removal. See the [release verification record](docs/Release0.3.0验收记录.md).

Global taxonomy lookups, synonyms, cultivar base taxa and existing-record enrichment remain available. The Beta 4 fixed sample returned both family and genus for 95 of 100 names; **global 99% coverage has not been achieved or established**. This historical sample is not a photo-identification accuracy measurement. Default font sizes are unchanged. See the [Beta 4 verification record](docs/Beta4验收记录.md).
...
7. Tap **Project repository** in Settings to open the source and release downloads in the system browser or installed GitHub app.
...
This stable APK is distributed through GitHub Releases; it has not been submitted to an app store. See the [bilingual release notes](docs/releases/v0.3.0.md) and [validation record](docs/Release0.3.0验收记录.md). Report issues with the app version, Android/device version, steps, and expected/actual results. Do not include keys or private photos.
